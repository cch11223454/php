...
require_once dirname(__FILE__) .'/BaiduOAuth2.php';
require_once dirname(__FILE__) .'/BaiduApiClient.php';
 ...
 * @version v2.0.0
 */
class baidu extends RelationModel{
    ...
    public function __construct($store = null)
    {
    	...
    	$this->redirectUri = $config['call_back'];
        $this->setStore($store ? $store : new BaiduCookieStore($this->clientId));
    	//$this->setStore=new BaiduCookieStore($this->clientId);
    }
    ...
    function get_code(){
		...
		$loginUrl = $this->getLoginUrl('', 'popup');
    ...
    function callback(){
    	$user = $this->getLoggedInUser();
    	if ($user) {
			$apiClient = $this->getBaiduApiClientService();
